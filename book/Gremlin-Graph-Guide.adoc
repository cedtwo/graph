...
upon. A 'filter' step will only pass on to the next step in the query if the incoming
elements meet the criteria specified within.
...
'false'. In the first example, the 'and' operator evaluates whether the sack's initial
value of 'true', and a constant value of 'false' are both 'true'.
